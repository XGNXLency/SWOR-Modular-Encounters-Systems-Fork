...
namespace ModularEncountersSystems.Spawning {
	...
	public static class BehaviorSpawnHelper {
		...
		private static void CompleteSpawning() {
            ...
            var parentCustomCountersVariables = _currentSpawn.ParentBehavior?.CurrentGrid?.Npc?.CustomCountersVariables ?? null;
            if (_currentSpawn.TransferCustomCountersVariables != null)
            {
                if (parentCustomCountersVariables != null)
                    foreach (var transferCounter in _currentSpawn.TransferCustomCountersVariables)
                    {
                        if (parentCustomCountersVariables.ContainsKey(transferCounter))
                            _currentSpawn.CustomCountersVariables[transferCounter] = parentCustomCountersVariables[transferCounter];
                    }
            }
            if (_currentSpawn.CustomCountersVariablesReferences != null)
            {
                if (parentCustomCountersVariables != null)
                    foreach (var transferCounter in _currentSpawn.CustomCountersVariablesReferences)
                    {
                        _currentSpawn.CustomCountersVariables[transferCounter.Key] = parentCustomCountersVariables[transferCounter.Value];
                    }
            }
			...
			if (_currentSpawn.SpawningType == SpawnTypeEnum.SpaceCargoShip) {
				...
				var result = SpawnRequest.CalculateSpawn(_spawnMatrix.Translation, _currentSpawn.ProfileSubtypeId, SpawningType.SpaceCargoShip, _currentSpawn.IgnoreSafetyChecks, _currentSpawn.ProcessAsAdminSpawn, _currentSpawn.SpawnGroups, _currentSpawn.CurrentFactionTag, parentId:_currentSpawn.ParentId, customCountersVariables: _currentSpawn.CustomCountersVariables);
			...
			if (_currentSpawn.SpawningType == SpawnTypeEnum.RandomEncounter) {
				...
				var spawns = _currentSpawn.SpawnGroups.Count > 0 ? _currentSpawn.SpawnGroups : null;
				var result = SpawnRequest.CalculateSpawn(_spawnMatrix.Translation, _currentSpawn.ProfileSubtypeId, SpawningType.RandomEncounter, _currentSpawn.IgnoreSafetyChecks, _currentSpawn.ProcessAsAdminSpawn, _currentSpawn.SpawnGroups, _currentSpawn.CurrentFactionTag, parentId:_currentSpawn.ParentId, customCountersVariables: _currentSpawn.CustomCountersVariables);
			...
			if (_currentSpawn.SpawningType == SpawnTypeEnum.PlanetaryCargoShip) {
				...
				var spawns = _currentSpawn.SpawnGroups.Count > 0 ? _currentSpawn.SpawnGroups : null;
				var result = SpawnRequest.CalculateSpawn(_spawnMatrix.Translation, _currentSpawn.ProfileSubtypeId, SpawningType.PlanetaryCargoShip, _currentSpawn.IgnoreSafetyChecks, _currentSpawn.ProcessAsAdminSpawn, _currentSpawn.SpawnGroups, _currentSpawn.CurrentFactionTag, parentId:_currentSpawn.ParentId, customCountersVariables: _currentSpawn.CustomCountersVariables);
			...
			if (_currentSpawn.SpawningType == SpawnTypeEnum.PlanetaryInstallation) {
				...
				var spawns = _currentSpawn.SpawnGroups.Count > 0 ? _currentSpawn.SpawnGroups : null;
				var result = SpawnRequest.CalculateSpawn(_spawnMatrix.Translation, _currentSpawn.ProfileSubtypeId, SpawningType.PlanetaryInstallation, _currentSpawn.IgnoreSafetyChecks, _currentSpawn.ProcessAsAdminSpawn, _currentSpawn.SpawnGroups, _currentSpawn.CurrentFactionTag, parentId:_currentSpawn.ParentId, customCountersVariables: _currentSpawn.CustomCountersVariables);
			...
			if (_currentSpawn.SpawningType == SpawnTypeEnum.BossEncounter) {
				...
				var spawns = _currentSpawn.SpawnGroups.Count > 0 ? _currentSpawn.SpawnGroups : null;
				var result = SpawnRequest.CalculateSpawn(_spawnMatrix.Translation, _currentSpawn.ProfileSubtypeId, SpawningType.BossEncounter, _currentSpawn.IgnoreSafetyChecks, _currentSpawn.ProcessAsAdminSpawn, _currentSpawn.SpawnGroups, _currentSpawn.CurrentFactionTag, parentId:_currentSpawn.ParentId, customCountersVariables: _currentSpawn.CustomCountersVariables);
			...
			if (_currentSpawn.SpawningType == SpawnTypeEnum.Creature) {
				...
				var spawns = _currentSpawn.SpawnGroups.Count > 0 ? _currentSpawn.SpawnGroups : null;
				var result = SpawnRequest.CalculateSpawn(_spawnMatrix.Translation, _currentSpawn.ProfileSubtypeId, SpawningType.Creature, _currentSpawn.IgnoreSafetyChecks, _currentSpawn.ProcessAsAdminSpawn, _currentSpawn.SpawnGroups, _currentSpawn.CurrentFactionTag, parentId:_currentSpawn.ParentId, customCountersVariables: _currentSpawn.CustomCountersVariables);
